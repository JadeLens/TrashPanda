...
/// <summary>
/// uses the any movement script that impletemts the IunitMovement 
/// </summary>
public class Node_MoveTo_With_Astar : aiBehaviorNode, IMoveToNode
{
	...
	public override void Act (GameObject ob)
	{
		...
		if (m_unit == null)
			m_unit = ob.GetComponent<IunitMovement> ();
...
//	private bool isStartLocationStored = false;
	...
	{
		//  Debug.Log(Vector3.Distance(m_target, ob.transform.position));
		if (Vector3.Distance (m_target, ob.transform.position) <= m_radius) {
			return true;
		} else {
			return false;
		}
...
/// <summary>
/// base movement for all steering behaviours Node  no nawMesh required
/// not to be used directly in the tree
/// </summary>
